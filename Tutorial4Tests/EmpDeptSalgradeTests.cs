...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        var emps = Database.GetEmps();
            ...
            var result = emps
        .Where(e => e.Comm != null) // Фільтруємо співробітників, у яких є комісія
        .Select(e => new { e.EName, e.Comm }) // Вибираємо тільки EName і Comm
        .ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps
            .Join(grades, 
                ...
                g => 1, 
                (e, g) => new { e, g }) 
            .Where(eg => eg.e.Sal >= eg.g.Losal && eg.e.Sal <= eg.g.Hisal) 
            .Select(eg => new { eg.e.EName, eg.g.Grade }) 
            .ToList(); 
        //
